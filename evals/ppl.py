...
class PerplexityEvaluator:
    ...
    def process_batch(self, batch: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        blocks = [(self.block_size-1)//self.bucket_size for _ in range(input_ids.shape[0])]
        return {
            'input_ids': input_ids,
            'loss': outputs['loss'],
            'nlls': nlls,
...
def main():
    ...
    evaluator = PerplexityEvaluator(
        ...
        block_size=args.block_size, bucket_size=args.bucket_size, batch_size=args.batch_size
    )
